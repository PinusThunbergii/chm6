    ...
    public partial class MainForm : Form
    {
        ...
        void CalculateButton_OnClick(object sender, EventArgs eventArgs)
        {
            try
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    dataGridView.Rows.Add(i, xi_array[i], yi_array[i], yi_array_mod[i], yi_array_anal[i]);
                    chart.Series[0].Points.AddXY(xi_array[i], yi_array[i]);
                    chart.Series[1].Points.AddXY(xi_array_anal[i], yi_array_anal[i]);
                    chart.Series[2].Points.AddXY(xi_array_mod[i], yi_array_mod[i]);
                }
